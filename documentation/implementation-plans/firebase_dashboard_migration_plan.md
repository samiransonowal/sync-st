...
We are replacing **Google Looker Studio** with a modern, cloud-native **Firebase-Hosted Web Application (`cineloom-comptroller`)**.
...
```mermaid
flowchart TD
    User[Samiran / Authorized Studio Admin] <-->|Google Auth & Role Check| FirebaseApp[Firebase Web App: cineloom-comptroller]
...
- Replace all references to Looker Studio with the **Firebase Comptroller Portal (`cineloom-comptroller`)**.
